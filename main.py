...
def plot_eeg_data(df, sampling_rate=128):
    ...
    for i, electrode in enumerate(electrodes):
        data = [x for x in df[electrode].astype(float) if x > 3900 and x < 4800]
    ...
    row_sums = [x/14 for x in df.astype(float).sum(axis='columns') if x/14<4500 and x/14>4200]
    # Generate time axis (assuming a uniform sampling rate)
